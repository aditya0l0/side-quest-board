 ...
 *
 * All XP derivation and status-transition rules live here,
 * keeping the controller thin and the entity clean.
 */
@Service
@Transactional
public class QuestService {
    ...
    /**
     * Create a new quest for today. XP is derived from difficulty;
    ...
    public List<QuestResponse> getQuestsForDate(LocalDate date) {
        return questRepository
                .findByQuestDateAndStatusNot(date, QuestStatus.ABANDONED)
                .stream()
    ...
    /**
     * Edit a quest's title, description, or difficulty.
     * Only allowed while the quest is ACTIVE.
     */
    public QuestResponse updateQuest(Long id, QuestRequest request) {
        Quest quest = findOrThrow(id);

        if (quest.getStatus() != QuestStatus.ACTIVE) {
            throw new IllegalQuestStateException(
                    "Cannot edit a quest that is " + quest.getStatus().name().toLowerCase()
    ...
    /**
     * Mark a quest as COMPLETED and lock it for further edits.
     * Awards XP by setting status to COMPLETED + recording completedAt.
     */
    public QuestResponse completeQuest(Long id) {
        Quest quest = findOrThrow(id);

        if (quest.getStatus() == QuestStatus.COMPLETED) {
            throw new IllegalQuestStateException(
                    "Quest is already completed. You can't claim XP twice!"
            );
        }
    ...
    /**
     * Soft-delete a quest by setting its status to ABANDONED.
     * It will be hidden from the active board but remains in history.
     */
    public QuestResponse abandonQuest(Long id) {
        Quest quest = findOrThrow(id);

        if (quest.getStatus() == QuestStatus.ABANDONED) {
            throw new IllegalQuestStateException(
                    "Quest is already abandoned."
            );
        }
        if (quest.getStatus() == QuestStatus.COMPLETED) {
            throw new IllegalQuestStateException(
    ...
    private Quest findOrThrow(Long id) {
